...
class EditUnidadModal(ctk.CTkToplevel):
    def __init__(self, master, unit_data, parent_view=None, **kwargs):
        ...
        self.old_nombre = unit_data.get('nombreUnidad')
        ...
        # Centrar + Offset derecha (dashboard style)
        ...
        self.lbl_title = ctk.CTkLabel(self.header, text="Editar Unidad de Medida", font=("Arial", 18, "bold"), text_color="white")
    ...
    def update_unidad(self):
        ...
        if not nuevo_nombre:
            Alerts.show_error("Campo requerido", "El nombre no puede estar vacío.", master=self)
            return
        ...
        resultado = UnidadService.update_unidad(self.id_unidad, nuevo_nombre)
        
        if resultado == "exists":
            Alerts.show_warning("Ya existe", f"Ya existe otra unidad con el nombre '{nuevo_nombre}'.", master=self)
        elif resultado:
            Alerts.show_success("Éxito", "La unidad ha sido actualizada.", master=self)
            if self.parent_view:
                self.parent_view.load_unidades()
            self.close_modal()
        else:
            Alerts.show_error("Error", "No se pudo actualizar la unidad.", master=self)
