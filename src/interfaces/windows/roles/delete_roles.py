...
class DeleteRoleModal(ctk.CTkToplevel):
    def __init__(self, master, role_data, parent_view=None, **kwargs):
        ...
        self.lbl_text = ctk.CTkLabel(
            self.container, 
            text=f"No podrás revertir la eliminación del rol:\n'{self.role_name}'", 
            font=("Arial", 16), 
            text_color="#545454",
            justify="center"
        )
    ...
    def animate_exit(self):
        ...
        try:
            # Obtener opacidad actual (o inicializar si falla)
            try:
            ...
            if alpha > 0:
                # Bajar la posición y reducir opacidad de forma más suave
                self.current_y += 10
    ...
    def confirm_delete(self):
        ...
        resultado = RoleService.delete_role(self.role_id)
        
        if resultado == "has_users":
            Alerts.show_error(
                "No se puede eliminar", 
                "Este rol tiene usuarios asociados. Primero debes mover o eliminar esos usuarios.", 
                master=self.master
            )
            self.close_modal()
        elif resultado:
            self.close_modal()
            Alerts.show_success("¡Eliminado!", f"El rol '{self.role_name}' ha sido borrado.", master=self.master)
            if self.parent_view:
                self.parent_view.load_role_cards()
                self.parent_view.load_statistics()
        else:
            Alerts.show_error("Error", "Ocurrió un fallo al intentar eliminar el rol.", master=self.master)
            self.close_modal()
