...
class DeleteUsuarioModal(ctk.CTkToplevel):
    def __init__(self, master, user_data, parent_view=None, **kwargs):
        ...
        self.lbl_icon_symbol = ctk.CTkLabel(
            self.icon_circle_frame, text="!", font=("Arial", 60, "bold"),
            text_color="#f8bb86"
        )
        ...
        self.lbl_text = ctk.CTkLabel(
            self.container, 
            text=f"No podrás revertir la eliminación del usuario:\n'{self.user_name}'", 
            font=("Arial", 16), 
            text_color="#545454",
            justify="center"
        )
        ...
        self.btn_confirm = ctk.CTkButton(
            ...
            font=("Arial", 15, "bold"), width=150, height=45,
            corner_radius=8,
        ...
        self.btn_cancel = ctk.CTkButton(
            ...
            font=("Arial", 15, "bold"), width=110, height=45,
            corner_radius=8,
    ...
    def animate_entry(self):
        """Animación de caída exponencial."""
    ...
    def animate_exit(self):
        """Animación de salida: desliza hacia abajo y desvanece suavemente."""
    ...
    def destroy_all(self):
        """Destruye de forma segura el modal y su capa de opacidad."""
        try:
    ...
    def confirm_delete(self):
        ...
        resultado = UsuarioService.delete_usuario(self.user_id)
        
        if resultado == "has_dependencies":
            Alerts.show_error(
                "No se puede eliminar", 
                "Este usuario tiene registros asociados (préstamos, bitácora, etc.).", 
                master=self.master
            )
            self.close_modal()
        elif resultado:
            self.close_modal()
            Alerts.show_success("¡Eliminado!", f"El usuario '{self.user_name}' ha sido borrado.", master=self.master)
            if self.parent_view:
                self.parent_view.load_user_cards()
                self.parent_view.load_statistics()
        else:
            Alerts.show_error("Error", "Ocurrió un fallo al intentar eliminar el usuario.", master=self.master)
            self.close_modal()
